...
class RestaurantRepository implements RestaurantRepositoryInterface {
    ...
    switch (source!) {
      ...
      case DataSourceEnum.local:
        ...
        if (cacheResponseData != null) {
          restaurantModel =
              RestaurantModel.fromJson(jsonDecode(cacheResponseData));
        }
    }
  ...
  @override
  Future<List<Restaurant>?> getCategoryWiseResturants(
      {required String categoryId, int limit = 10, int offset = 0}) async {
    List<Restaurant>? categoryWiseRestaurents;

    String cWALUri =
        "${AppConstants.categoryWiseResturentUri}/$categoryId?limit=$limit&offset=$offset";
    Response response = await apiClient.getData(cWALUri);
    debugPrint('resturents data ${response.body}');
    if (response.statusCode == 200) {
      categoryWiseRestaurents = [];
      List<dynamic> data = response.body['restaurants'];
      data.forEach((restaurant) {
        categoryWiseRestaurents!.add(Restaurant.fromJson(restaurant));
      });
    }

    return categoryWiseRestaurents;
  }
}
